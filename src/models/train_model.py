...
if __name__ == '__main__':
    ...
    X = X[y_train == 0][np.random.randint(0, 30000, size=1000)]  # get only normal training examples

    # model = AutoEncoder(epochs=10)
    # model.fit(X)
    # 
    # test_indices = np.random.randint(500000, 900000, size=10000)
    # y_pred = model.predict(X_val[test_indices])
    # visualize_distribution_with_labels(y_pred, y_val[test_indices])
