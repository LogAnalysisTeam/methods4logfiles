...
def get_data_by_indices(data: defaultdict, labels: pd.DataFrame) -> Dict:
    ret = {block_id: data[block_id] for block_id in labels['BlockId']}
    return ret


def train_test_split(data: defaultdict, labels: pd.DataFrame, test_size: float, seed: int) -> tuple:
