...
class TCNCNN1DPyTorch(nn.Module):
    def __init__(self, input_dim: int, window: int, layer_configurations: List, encoder_kernel_size: int,
                 tcn_kernel_size: int, decoder_kernel_size: int, dropout: float, maxpool: int = 2,
                 upsampling_factor: int = 2):
        super().__init__()

        layers = [nn.Conv1d(input_dim, layer_configurations[0][0], kernel_size=encoder_kernel_size), nn.ReLU(),
                  nn.MaxPool1d(maxpool)]
        for i in range(1, len(layer_configurations[0])):
            layers.append(
                nn.Conv1d(layer_configurations[0][i - 1], layer_configurations[0][i], kernel_size=encoder_kernel_size))
            layers.append(nn.ReLU())
            layers.append(nn.MaxPool1d(maxpool))

        layers.append(TemporalConvNet(layer_configurations[0][-1], layer_configurations[1], kernel_size=tcn_kernel_size,
                                      dropout=dropout, include_last_relu=True))

        for i in range(len(layer_configurations[2])):
            neurons_in_prev_layer = layer_configurations[1][-1] if i == 0 else layer_configurations[2][i - 1]
            layers.append(nn.ConvTranspose1d(neurons_in_prev_layer, layer_configurations[2][i],
                                             kernel_size=decoder_kernel_size))
            layers.append(nn.ReLU())
            layers.append(nn.Upsample(scale_factor=upsampling_factor))

        neurons_in_prev_layer = layer_configurations[2][-1] if layer_configurations[2] else layer_configurations[1][-1]
        layers.append(nn.ConvTranspose1d(neurons_in_prev_layer, input_dim, kernel_size=decoder_kernel_size))
...
class TCNCNN1D(sklearn.base.OutlierMixin):
    ...
    def fit(self, X: np.ndarray) -> TCNCNN1D:
        # 1. convert to torch Tensor
        train_dl = self._numpy_to_tensors(X, batch_size=self.batch_size, shuffle=True)

        # 2. initialize model
        if not self._model:
            self._initialize_model(X[0].shape[-1], [[64], [128], [64]], 3, 3, 5, 0.3)
    ...
    def set_params(self, **kwargs):
        ...
        self._initialize_model(kwargs['input_shape'], kwargs['layers'], kwargs['encoder_kernel_size'],
                               kwargs['tcn_kernel_size'], kwargs['decoder_kernel_size'], kwargs['dropout'])

    def _initialize_model(self, input_shape: int, layers_out: List, encoder_kernel_size: int, tcn_kernel_size: int,
                          decoder_kernel_size: int, dropout: float):
        self._model = TCNCNN1DPyTorch(input_shape, self.window, layers_out, encoder_kernel_size, tcn_kernel_size,
                                      decoder_kernel_size, dropout)
